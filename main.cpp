...
struct Init {
  Init() {
    if (SDL_Init(SDL_INIT_VIDEO | SDL_INIT_AUDIO) != 0) {
      ...
      stream << "SDL could not initialize! SDL_Error: " << SDL_GetError();
...
struct Window {
  Window()
    ...
    if (window == nullptr) {
      std::stringstream stream;
      stream << "Window could not be created! SDL_Error: " << SDL_GetError();
      throw std::runtime_error{stream.str()};
    }
  }
...
struct Renderer {
  explicit Renderer(SDL_Window *window)
    ...
    if (renderer == nullptr) {
      std::stringstream stream;
      stream << "Renderer could not be created! SDL_Error: " << SDL_GetError();
      throw std::runtime_error{stream.str()};
    }
  }
...
struct Texture {
  Texture(SDL_Renderer *renderer, SDL_Surface *surface)
    ...
    if (texture == nullptr) {
      std::stringstream stream;
      stream << "Unable to create texture! SDL_Error: " << SDL_GetError();
      throw std::runtime_error{stream.str()};
    }
  }
...
struct ImageSurface {
  ImageSurface(const std::string &imagePath)
